...
const statusOptions = ["Available", "Break Stick", "Queue Maintenance", "Lunch"];
...
function displayStatusOptions(member) {
    ...
    const memberStatusOptions = teamMembers[member].position === "Supervisor" ? supervisorStatusOptions : statusOptions;
    ...
    if (isOnLeft) {
        ...
        optionsContainer.style.right = `${window.innerWidth - rect.left + window.scrollX}px`;
    } else {
        optionsContainer.style.top = `${rect.top + window.scrollY}px`;
...
function displaySupervisorStatusOptions(member) {
    ...
    supervisorStatusOptions.forEach(option => {
        ...
        if (option === "Break") {
            ...
            optionButton.style.backgroundColor = "#FFD95A";
        }
...
function updateStatus(member, newStatus, backgroundColor) {
    ...
    if (teamMembers[member].position === "Supervisor") {
        ...
        // Check for specific background colors for "Break" and "Meeting" statuses
        if (newStatus === "Break") {
    ...
    const breakStickOccupied = Object.values(teamMembers).some(
        (m) => m.status === "Break Stick" && m !== teamMembers[member]
    );

    // Check if a member is already on "Queue Maintenance" status
    const queueMaintenanceOccupied = Object.values(teamMembers).some(
        (m) => m.status === "Queue Maintenance" && m !== teamMembers[member]
    );

    // Check if the maximum number of members on "Lunch" has been reached
    const lunchOccupied = Object.values(teamMembers).filter(
        (m) => m.status === "Lunch"
    ).length >= 3;
    ...
    if (
        ...
        (newStatus === "Queue Maintenance" && queueMaintenanceOccupied)) &&
        teamMembers[member].position !== "Supervisor" // Supervisors are exempt from these rules
    ) {
        alert(
            `Sorry, someone is already on ${newStatus} status.`
        );
    ...
    if (newStatus === "Lunch" && lunchOccupied) {
        alert("Sorry, the maximum number of team members on lunch has been reached.");
        return;
    }
    ...
    if (member === "duo" || member === "buzzworthy" || member === "assisted") {
        ...
        document.getElementById(member).innerText = `${teamMembers[member].position} - ${member.charAt(0).toUpperCase() + member.slice(1)} - ${newStatus}`;
    }

    // Update the background color of the member button based on the status
    const memberButton = document.getElementById(member);
    memberButton.classList.remove("available", "breakstick", "queuemaintenance", "lunch", "duo", "buzzworthy", "assisted");
